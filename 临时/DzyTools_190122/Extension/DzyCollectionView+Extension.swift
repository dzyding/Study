...
extension UICollectionView {
    ...
    func dzy_dequeueHeader<T: UICollectionViewCell>(_ cls:T.Type, _ index: IndexPath) -> T? {
        let name = String(describing: cls)
        return dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: name, for: index) as? T
    }
    
    func dzy_dequeueFooter<T: UICollectionViewCell>(_ cls:T.Type, _ index: IndexPath) -> T? {
        let name = String(describing: cls)
        return dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: name, for: index) as? T
    }
}
