...
class DzyAdView: UIView {
    ...
    private func scrollInitStep(frame: CGRect) {
    ...
    private func pageInitStep() {
        guard urlArr.count > 1, let scrollFrame = scrollView?.frame else {return}
        switch pageType {
        case .rect:
            let width:CGFloat = (8.0 + 6.0) * CGFloat(urlArr.count) - 6.0
            let frame = CGRect(
                x: ScreenWidth - 18.0 - width,
        ...
        case .num:
            let frame = CGRect(
                x: ScreenWidth - 18.0 - 40.0,
                y: scrollFrame.height - 12.0 - 6.0,
                width: 40.0,
                height: 12.0
            )
