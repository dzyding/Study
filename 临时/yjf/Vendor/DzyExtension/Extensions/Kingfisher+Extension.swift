...
// 占位图的名字
private let placeholderName = "temp"
...
extension UIImageView {
    func dzy_setCircleImg(_ url: String, _ size: CGFloat) {
        if let url = URL(string: url) {
            kf.setImage(
                with: url,
                placeholder: Image(named: placeholderName),
                options: [.cacheOriginalImage, .imageModifier(RoundModifier(size))]
            )
        }
    }
...
extension UIButton {
        ...
        if let url = URL(string: url) {
            ...
            DispatchQueue.main.async {
                if let placeholder = placeholder {
                    let image = placeholder.kf.image(
                        withRoundRadius: placeholder.size.height / 2.0,
                        fit: placeholder.size
                    )
    ...
    func dzy_setImg(
        _ url: String,
        placeholder: UIImage? = UIImage(named: placeholderName),
