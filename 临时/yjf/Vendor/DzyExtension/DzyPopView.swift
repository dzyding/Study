...
class DzyPopView: UIView {
    ...
    private let type:DzyPopType
    
    private var originFrame:CGRect = .zero
    /// 点击背景隐藏时触发的事件
    var bgDismissHandler: (()->())?
    /// 背景是否可以点击
    var isBgCanClick: Bool = true
    ...
    func updateSourceView(_ v: UIView) {
        ...
        sourceView.flatMap({$0.removeFromSuperview()})
        sourceView = v
        setSource(type)
    }
    ...
    override func didMoveToSuperview() {
        ...
        default:
            ...
            UIView.animate(
                ...
                animations:
                {
                    sourceView.transform = sourceView.transform.translatedBy(
                        x: 0,
                        y: sourceView.frame.size.height
                    )
            }, completion:nil)
        }
    }
    ...
    ///设置内容
    private func setSource(_ type:DzyPopType) {
        guard let sourceView = sourceView else {return}
