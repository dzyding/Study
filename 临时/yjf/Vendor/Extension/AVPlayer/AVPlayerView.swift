...
class AVPlayerView: UIView {
    ...
    func setPlayerSourceUrl(url:String) {
        ...
        queryCacheOperation = WebCacheManager.shared()
            .queryURLFromDiskMemory(
                key: cacheFileKey ?? "",
                cacheQueryCompletedBlock:
        { [weak self] (data, hasCache) in
            DispatchQueue.main.async {[weak self] in
                if !hasCache {
                    if let finalUrl = self?.sourceURL {
    ...
    func setPlayerAdUrl(url: String) {
    ...
    func cancelLoading() {
        ...
        if player != nil
        {
            AVPlayerManager.shared().remove(player: player!)
        ...
        cancelLoadingQueue?.async {[weak self] in
            ...
            for loadingRequest in self?.pendingRequests ?? [] where !loadingRequest.isFinished {
    ...
    func getVideoSize(_ complete:@escaping (()->())) {
        if videoSize == nil {
            self.urlAsset?.loadValuesAsynchronously(forKeys: ["tracks"], completionHandler: {
                for track in (self.urlAsset?.tracks)! where track.mediaType == AVMediaType.video {
                    self.videoSize = track.naturalSize
                    let t = track.preferredTransform
                    if t.a == 0 && t.b == 1.0 && t.c == -1.0 && t.d == 0 {
                        //Portrait
                        self.videoSize = CGSize(width: self.videoSize?.height ?? 0, height: self.videoSize?.width ?? 0)
                    }
                    complete()
                    break
    ...
    func seekTo(_ value:Float)
    {
        ...
        // 视频要切换的时间
        let toTime = sumTime * value
        
        // 切换时间
        player?.seek(to: CMTimeMakeWithSeconds(Float64(toTime), preferredTimescale: Int32((player?.currentItem?.duration.timescale)!)), completionHandler: { (Bool) in
            self.player?.play()
        })
...
extension AVPlayerView {
    ...
    private func adAddProgressObserver() {
        timeObserver = player?.addPeriodicTimeObserver(forInterval: CMTime.init(value: 1, timescale: 1), queue: DispatchQueue.main, using: {[weak self] time in
            let current = CMTimeGetSeconds(time)
            let total = CMTimeGetSeconds(self?.playerItem?.duration ?? CMTime.init())
            self?.delegate?.onProgressUpdate(current: CGFloat(current), total: CGFloat(total))
