...
class DzyShowImageCell: UICollectionViewCell {
    ...
    weak var scrollView: DzyShowImageScrollView?
...
extension DzyShowImageCell: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        let y = -scrollView.dzy_ofy
        if y >= 100,
            let v = superview?.superview as? DzyShowImageView
        {
            v.hidden()
        }
    }
