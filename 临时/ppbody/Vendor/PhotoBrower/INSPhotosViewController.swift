...
open class INSPhotosViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIViewControllerTransitioningDelegate {
    ...
    open func deletePhoto(_ photo: INSPhotoViewable)
    {
        let index = dataSource.indexOfPhoto(photo)
        
        var nextPhoto = dataSource.photoAtIndex(index!+1)
        if nextPhoto == nil{
            nextPhoto = dataSource.photoAtIndex(index!-1)
        }
        ...
        changeToPhoto(nextPhoto!, animated: true)
    }
