...
class CourseCropVideoVC: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        ToolClass.removeChildController(self, removeClass: CourseVideoRecordVC.self)
        
        self.cutInfo.minDuration = 4
        self.cutInfo.maxDuration = 15
    ...
    func didStartClip()
    {
        ...
        {
            self.delegate?.recordFinfish(cutInfo.path!)
...
extension CourseCropVideoVC: CropVideoThumbnailViewDelegate,AliyunCropDelegate
{
    ...
    func cropTaskOnComplete() {
        ...
        if isCancel
        {
            ...
            self.delegate?.recordFinfish((cutPanel?.outputPath)!)
            self.dismiss(animated: true, completion: nil)
