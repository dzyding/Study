...
class TribeTrendsChildVC: BaseVC {
    ...
    convenience init(itemInfo: IndicatorInfo) {
        self.init()
        if itemInfo.title == "热门" {
            self.type = "10"
        }
        else if itemInfo.title == "最新" {
            self.type = "20"
        }
    ...
    func getData(_ page: Int)
    {
        ...
        request.url = BaseURL.TopicForTribeId
        request.isUser = true
...
extension TribeTrendsChildVC: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,IndicatorInfoProvider,TopicDetailScrollDelegate {
